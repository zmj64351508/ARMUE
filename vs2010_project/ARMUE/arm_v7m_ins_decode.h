...
typedef _armv7m_translate16_t (*armv7m_translate16_t)(uint16_t opcode, struct cpu_t* cpu);
...
typedef struct {
	...
	armv7m_translate16_t		base_table16[BASE_TABLE_SIZE_16];

	// all the sub-tables stores the appropriate implement function pointer of the instruction
	armv7m_translate16_t		shift_add_sub_mov_table16[SHIFT_ADD_SUB_MOV_SIZE_16];
	armv7m_translate16_t		data_process_table16[DATA_PROCESS_SIZE_16];
	armv7m_translate16_t		spdata_branch_exchange_table16[SPDATA_BRANCH_EXCHANGE_SIZE_16];
	armv7m_translate16_t		load_literal_table16[LOAD_LITERAL_SIZE_16];
	armv7m_translate16_t		load_store_single_table16[LOAD_STORE_SINGLE_SIZE_16];
	armv7m_translate16_t		pc_related_address_table16[PC_RELATED_ADDRESS_SIZE_16];
	armv7m_translate16_t		sp_related_address_table16[SP_RELATED_ADDRESS_SIZE_16];
	armv7m_translate16_t		misc_16bit_ins_table[MISC_16BIT_INS_SIZE];
}armv7m_instruct_table_t;
